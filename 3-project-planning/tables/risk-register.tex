...
\hl{TC80}& \hl{Moving parts jamming} & \hl{B} & \hl{3} & \cellcolor[HTML]{FCFF2F} Low & \hl{Moving parts will be properly cleaned and lubricated where needed.} \\\hline 
...
MS20 & Damage to the system on landing										& D & 1 & \cellcolor[HTML]{FCFF2F}Low			& \st{Optics will move inside a frame for protection} \hl{Optics will rotate into a position where damage can be avoided/minimized}.\\\hline
...
\hl{MS80} & \hl{Scientific objective not achieved} & \hl{B} & \hl{3} & \cellcolor[HTML]{FCFF2F} Low & \hl{Critical components for the pointing system will be selected, designed and manufactured with a high enough safety factor.}\\\hline
...
\st{VE20} & \st{Collision of telescope with landing mechanism}						& \st{A} & \st{4} & \cellcolor[HTML]{34FF34}\st{Very Low}	& \st{The experiment will be inside a frame for protection.}\\\hline
