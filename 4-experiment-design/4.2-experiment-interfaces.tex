...
\label{sec:4.2.3}
\textbf{E-link:}\\
The uplink will be used to send occasional commands to control the experiment. One command will be [TBD] in size. The TCP/IP protocol will be used for uplink, so the total size of one transmission will be the data plus the TCP header (maximum 480 bits), the IP header (maximum 480 bits) and the Ethernet frame (144 bits).

$$ bits\, per\, uplink\, packet \leq [TBD] + 480 + 480 + 144 $$

The downlink will be used to send science and housekeeping data to the ground station. The data in the downlink packet is estimated to be [TBD]. For downlink, \st{UDP} \hl{TCP/IP} protocol will be used, \st{as the reliability of the data is not as crucial as during the uplink and to compensate for the size of the data packet} \hl{to ensure the delivery of mission critical housekeeping data}. The total size of one packet will be the data plus the \st{UDP header (32 bits), the IP header (maximum 480 bits) and the Ethernet frame (144 bits)} \hl{TCP header (maximum 480 bits), the IP header (maximum 480 bits) and the Ethernet frame (144 bits)}.

$$ bits\, per\, downlink\, packet \leq [TBD] + 480 + 480 + 144 $$
\begin{center}
\hl{[Changed the values in the equation to fit TCP/IP protocol.]}
\end{center}
